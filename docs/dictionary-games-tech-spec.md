...
- **Progress sync (Phase 3, 2026-08-05): implemented, gated, dormant in
  ...
  §6b, §9) — the missing piece is now **engine-side**, not an unbuilt
  issuer: `sparxstar-identity` exists and mints suite tokens, but
  `/events/batch` does not yet accept them (§11). It is not a guest-token
  question either (guest play never reaches this path at all, by design).
  No host does this yet and the path is inert in production today. The `aiwa_game_*` bonus markers (streak, first-practice,
...
| ID    | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ----- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| —     | **Guest-sync question: CLOSED by design. Adult sync: approved, engine-side work outstanding (corrected 2026-08-22).** Earlier framings here (and via the retired "OQ-G1" label) treated this as "anonymous/guest game clients have no token-issuance mechanism that fits." Per `3IATLAS-IDENTITY-AND-GAME-SERVICES-DECISION-v1.0.md` §4 (`sparxstar-3iatlas-rlc-node-engine` repo — read directly, not relayed): guest play is device-local by design, permanently; guests are never issued a token, so there was never anything to design for them. That half is closed and will not reopen. **The issuer half is also no longer missing:** `sparxstar-identity` exists as `Starisian-Technologies/sparxstar-3iatlas-identity-node` — it mints RS256 suite tokens (`sub`, `iss`, `iat`, `exp`, `account_id`, `school_id`, `tier`; deliberately no `scope` claim — it states identity, never permission), publishes JWKS at `/.well-known/jwks.json` for consumers to verify against, and its own production-readiness review (2026-08-19) rates it ready for the adult account tier, with a documented deploy path. So this document's previous claim that the issuer "doesn't exist yet" is **wrong and is retracted here.** The adult suite-token path into the Game Node is **approved** — R3 (2026-08-01) ruled `game.result` intake authenticated-only, and GAME-SERVICE-INTAKE-SPEC-v1.0 §3 states that a standalone/solo player uses the suite token rather than a participant token. What remains is **engine-side implementation, not a decision and not an issuer**: (1) `/events/batch` is gated on `requireParticipant` (HMAC, `verifyParticipantToken` against `config.secrets.participant`), so a genuine suite token is rejected there today and must be admitted by a **separate** suite-token middleware verifying Identity's JWKS — never by routing suite tokens through `requireParticipant`; and (2) `settleGameResult` resolves `getSession(participant.session_id)` and throws `session_unavailable`, then reads `class_id`/`school_id` off that session to grant XP, so an adult solo player with no RLC session needs a session-less settlement path. Until both land, `syncNow()` stays inert in production — the client side is complete and integration-tested. Not a launch blocker for adult games: play, scoring and progress are device-local and fully functional without it. |
