...
- **Progress seam (implemented, gated):** `useProgressSync.addEvent` writes to
  ...
  for _authenticated_ accounts, so no host has a real token to inject.
