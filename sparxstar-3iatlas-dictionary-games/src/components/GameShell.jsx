...
const useGameSession =
  useGameSessionModule.useGameSession ?? useGameSessionModule.default;
if (!useGameSession) {
...
export default function GameShell({
  ...
  const useProgressSync =
    useProgressSyncModule.useProgressSync || useProgressSyncModule.default;
    ...
    const load = async () => {
      try {
        ...
        if (lastVisit.available && lastVisit.value !== today) {
          const didPersistVisit = setLocalStorageItem(
            "aiwa-dict-last-play",
            today,
          );
          if (didPersistVisit) {
  ...
  const handleComplete = useCallback(async () => {
    ...
    await addEvent({
      type: "aiwa_game_session_complete",
      domain: selectedDomain,
    });
    await syncNow();
    setPhase("complete");
