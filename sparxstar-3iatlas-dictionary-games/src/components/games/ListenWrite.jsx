...
export default function ListenWrite({ words, language, onResult, onComplete }) {
  const deck = useMemo(
    () => shuffle(words.filter((w) => !!w.audio_url)),
    [words],
  );
    ...
    <div className="flex flex-col h-full p-4 gap-4">
      ...
      <div className="shrink-0 flex flex-col items-center gap-3 py-4">
        <button
          ...
          style={{
            background: "linear-gradient(135deg, #E91E8C 0%, #7B3FA0 100%)",
          }}
      ...
      {/* IPA */}
      {word.ipa && (
        <p className="text-center text-sm font-mono text-gray-400 shrink-0">
          /{word.ipa}/
        </p>
      )}
